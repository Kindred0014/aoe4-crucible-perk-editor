...
def apply_change(value, log, dry_run=False, debug=False):
    ...
    pending_value = state.get("scan_value")
    pending_kind = state.get("scan_kind", "int64")
    pending_hits = state.get("scan_hits") or []
    if pending_hits and pending_value is not None and pending_value != value:
        alive = [int(text, 16) for text in pending_hits
                 if read_number(handle, int(text, 16), pending_kind) == value]
        note("  上次 %d 存了 %d 个候选，按新值 %d 过滤后剩 %d 个"
             % (pending_value, len(pending_hits), value, len(alive)))
        if alive:
            address = pick_target(handle, alive, note)
            if address is not None:
                chosen = (address, pending_kind)

    if not chosen:
        for kind in ("int64", "int32"):
            hits = scan_value(handle, value, kind, note)
            hits_by_kind[kind] = hits
            total_hits += len(hits)
            if not hits:
                continue
    ...
    if not chosen:
        ...
        if stored:
            ...
            if (slot_signature(handle, stored_address)
                ...
                note("  用上次记录的地址命中: 0x%X" % stored_address)
    ...
    if not chosen:
        ...
        address_kind = "int64" if hits_by_kind.get("int64") else "int32"
        if not dry_run:
            state["scan_value"] = value
            state["scan_kind"] = address_kind
            state["scan_hits"] = ["0x%X" % hit for hit in hits_by_kind[address_kind]]
            save_state(state)
        return False, ("先在游戏里让点数变化一次：\n"
                       "买一个天赋（或退一个），\n"
                       "再把新数值填进来，点一次按钮。")
    ...
    state.pop("scan_value", None)
    state.pop("scan_kind", None)
    state.pop("scan_hits", None)
    save_state(state)
